...
const DEFAULT_WAIT_MS    = 0; // parseInt(process.env.MOCK_WAIT_MS ?? 0, 10);
...
app.post('/v2/auth/sa/oauth/token', (_req, res) => {
  res.json({
...
// ─── Detokenize ───────────────────────────────────────────────────────────────
// POST /v2/vaults/:vaultId/detokenize
// Works for single and bulk — array size in detokenizationParameters.

app.post('/v2/vaults/:vaultId/detokenize', async (req, res) => {
  metrics.totalRequests++;
...
app.listen(PORT, () => {
  ...
  console.log('Routes:');
  console.log('  POST /v2/auth/sa/oauth/token');
  console.log('  POST /v2/vaults/:vaultId/:table       (insert)');
  console.log('  POST /v2/vaults/:vaultId/detokenize   (detokenize)');
