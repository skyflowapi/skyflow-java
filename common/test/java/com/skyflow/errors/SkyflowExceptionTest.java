...
public class SkyflowExceptionTest {
    ...
    public void testConstructorWithJsonErrorBody() {
        ...
        Assert.assertFalse(ex.getDetails().isEmpty());
    }
    ...
    public void testConstructorWithNullErrorBody() {
        ...
        Assert.assertEquals(ex.getMessage(), "fail");
        Assert.assertEquals(ex.getHttpStatus(), HttpStatus.BAD_REQUEST.getHttpStatus());
        Assert.assertNull(ex.getGrpcCode());
    }
    ...
    public void testSetDetailsWithErrorFromClientHeader() {
        String jsonString = "{\"error\":{\"message\":\"test error\",\"grpc_code\":13,\"details\":[]}}";
        Map<String, List<String>> headers = new HashMap<>();
        headers.put("error-from-client", Collections.singletonList("client error"));

        SkyflowException ex = new SkyflowException(500, new RuntimeException("fail"), headers, jsonString);
